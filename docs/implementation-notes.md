...
- Global auth switching, locking, backup, and restore in `internal/multicodex/switch_global.go`.
- Codex Mac app launch handling in `internal/multicodex/app_launch.go`.
...
- `<default-codex-home>/auth.json.lock` for serializing shared default auth pointer changes.
...
- Unit tests for global switch locking, atomic replacement, backup, and restore behavior.
- Unit tests for Codex Mac app launch environment cleanup and failed-launch auth restore.
