...
Trade-offs: Slightly more checks and warnings in doctor and status output. The multicodex state home, profile directories, profile `auth.json`, profile `codex-home`, and profile skills directory must be regular profile-local filesystem entries; `config.toml` may still be a symlink to shared default config.
Enforcement: Doctor checks for path isolation, ignore coverage, tracked sensitive files, auth-file symlinks, auth-file hard links, and symlinked profile homes. Setup, status, shell switching, monitor profile discovery, and shared profile execution preflights reject unsafe profile paths before running Codex; config saves use exclusive temp files; profile skill repair rejects symlinked skill directories; heartbeat lock files reject symlinks and hard links; login normalizes regular auth files to `0600`; tests cover helper logic.
