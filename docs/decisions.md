...
Enforcement: All disk preflight checks route through the shared parser, and every managed login, interactive CLI, configured-profile exec, and heartbeat child also receives a shared final `-c 'cli_auth_credentials_store="file"'` override before standalone `--`. Default-account exec and exact exec-help delegation remain unchanged. Regression tests cover parser edge cases, user config and profile overrides, delimiter placement, each managed launch path, and the default/help exclusions.
