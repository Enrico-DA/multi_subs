...
Trade-offs: Selection is snapshot-based, so simultaneous launches can still choose the same account and the chosen account may change between invocations. Red five-hour accounts can still be selected when they have usage left, so a very large prompt may exhaust that five-hour window mid-run. The protected default account can also be used as the final fallback even when its usage data is unavailable or exhausted; that accepts possible Codex-side failure to satisfy the rule that a prompt should be sent somewhere when any destination exists.
Enforcement: `multicodex exec` forwards all arguments directly to `codex exec`, bypasses profile selection only for exact help requests, and excludes configured profiles at 100% five-hour or weekly usage. Configured profiles are grouped by five-hour usage: green is 0-40%, amber is 41-60%, and red is 61-99%. Exec tries green profiles before amber profiles, amber profiles before red profiles, and within each tier picks the profile whose weekly reset is soonest. The default Codex home is a protected reserve and is used only when no configured profile has current usable five-hour and weekly usage left, including when there are no configured profiles. If the default Codex home is the only remaining destination, exec uses it as the final fallback even when its usage data is unavailable or exhausted. Unsafe configured profile paths still fail before routing instead of being hidden by global fallback. Tests assert tier ordering, hard exclusion of exhausted configured profiles, protected default-reserve execution, red-profile selection for the current usage shape, no-profile reserve execution, and final reserve fallback.
...
Decision: Use Codex app-server as the normal monitor usage source, with direct OAuth as fallback.
...
Rationale: The monitor should match Codex CLI auth handling for valid profile-local logins instead of treating refreshable credentials as logged out. Keeping OAuth as fallback preserves a simpler direct path when app-server startup fails.
Trade-offs: Normal monitor refreshes may start profile-scoped read-only Codex app-server sessions. This is heavier than direct OAuth, but it avoids false `auth expired` rows for profiles that Codex itself can still use.
Enforcement: Account fetchers use app-server first and OAuth fallback; `monitor doctor` checks the same normal usage path by default and only adds separate raw app-server checks when `--app-server` is passed. Exit status succeeds when at least one usage fetch works, with degraded status when another fetch or setup check fails.
References: `internal/monitor/usage/fetcher.go`, `internal/monitor/usage/source.go`, `internal/monitor/usage/doctor.go`, `internal/monitor/usage/fetcher_test.go`, `README.md`, `docs/command-spec.md`
...
Rationale: Using Spark windows for Spark model names gives better quota fit for model-appropriate routing, while still preserving the rule that the default Codex home is the final destination when no configured profile can take the prompt.
...
Enforcement: `internal/multicodex/exec.go` passes parsed model to `usage.SelectBestAccountForModel`; model parsing and selection tests assert parse flow, Spark-window selection for configured profiles, and final default-reserve fallback when configured Spark windows are missing.
