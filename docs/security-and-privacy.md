...
- The default Claude account is usage-visible and explicitly runnable with `multisubs claude cli default`. It is not an automatic routing candidate, and its cached identity and organization are never trusted.
...
`multisubs codex generate` uses the selected home's existing managed ChatGPT authentication through Codex App Server and pins Codex's built-in OpenAI provider. It supports Codex CLI 0.147.0 and 0.148.0, ignores unrelated custom model providers, disables notification hooks, fails closed if config replaces the built-in provider or its endpoint, loads a configuration lockfile, or changes the selected search mode, requests no token refresh, rejects API-key authentication, discards App Server standard error, and never reads or prints credential values. Generation workspaces and model catalogs are private temporary files. Generation reads the bundled model catalog without an authenticated model-discovery request, verifies its effective provider, endpoint, catalog, hooks, MCP state, and search mode before account access, and uses an empty working directory, explicit file-backed client instructions or empty defaults, disabled context, an ephemeral thread, read-only sandboxing, and fail-closed handling for action requests or unexpected items. Generation exposes no tools by default. Explicit `--search` exposes only native live web search, rejects inherited nested search settings such as domain or approximate-location filters, and accepts search events only during that opted-in run. Native search sends prompt-derived queries and page requests through the selected subscription provider. Generation input files are read only from paths selected by the caller and are bounded independently. File-read errors do not print those paths. JSON result mode buffers a bounded response and exposes only response text, model, effective effort, elapsed milliseconds, a count of distinct native web-search items, and numeric token usage.

Managed Claude usage treats an official not-logged-in auth status as `not logged in` even when the `/usage` probe returns unreadable text. A first official logged-out result skips `/usage`. Default Claude usage does not query cached auth status; it runs only `/usage`, whose fixed login-required result also becomes `not logged in`. These paths point Next at the matching login command instead of `multisubs doctor`. Direct Claude status still checks default credential presence. It prints `identity unavailable` for a logged-in default, hides cached auth metadata, and adds no Next step. Doctor warns that the default identity cannot be verified; that warning alone is not a failed check.

The unified usage report reads exactly the managed profiles in the two provider registries plus both normal default accounts. Shared typed target enumeration keeps the Codex default present exactly once even when a stored managed home is unsafe; that unsafe managed entry fails only its own row. Claude usage derives its targets from the shared Claude reporting target owner. Automatic Claude execution separately enumerates managed profiles only. Neither path reads monitor account files, active-home overrides, or discovered accounts. Usage does not create multisubs directories or persist identity in config. The official unmanaged default Codex app server may write non-credential logs, caches, database files, and database write-ahead files in the default home.

Codex normalization retains one declared short/session window for reporting while routing and monitor sources continue to consume weekly fields only. A declared 300-minute window is the five-hour session. Otherwise only one unambiguous declared non-weekly duration is accepted; response position is never used to guess session meaning. Session-only primary data never suppresses weekly fallback or enables routing or selection metadata. The report-only source may merge that session into a fallback weekly result. Without weekly data the account remains partial and exit code 1 applies.

The user explicitly requested account identities in local usage output. The three usage commands therefore print a full validated account email beside each identified logical row by default. This exception is limited to local usage output and never applies to the default Claude target. Account ID, user ID, organization ID, tokens, paths, raw identity conflicts, provider bodies, subprocess failures, cleanup errors, and unknown provider limit names are never printed there. Codex extra limits use only fixed product-owned labels, currently `Spark weekly`. When a usage or status snapshot is not complete, a Next section may print each distinct exact command once from a closed allow-list; the first failed account that needs a command supplies that row's label and reason. Profile names in those commands are validated registry names. Default-account login commands are the official provider CLIs. Next-step reasons are the same fixed safe failure phrases already used in the quota rows.

Codex reconciliation uses non-empty account ID as the strongest opaque internal key. A strict normalized email is the only fallback; user ID alone is never used. Different non-empty account IDs never merge merely because email matches. One email-only record may join one strong record with the same email. If it could match several strong IDs, it is conflicted and cannot add routing capacity. A failed Codex usage probe may recover only that normalized email through the existing protected auth-file reader, except after the unmanaged default app-server path is chosen. That path performs no later auth-file identity read. Managed Claude usage identity requires official logged-in status, non-empty organization ID, and validated email without applying routing-only Max, provider, or method rules. One deadline covers auth, usage, and auth again. Different non-empty managed organization IDs never merge by email, and identity that changes or fails the second check remains ungrouped. The default Claude usage path never queries cached email or organization because those fields cannot prove which live credential context supplied `/usage`.

Duplicate logical subscriptions produce one usage row and one availability count. Managed aliases sort deterministically. A Codex group containing the default account renders `(also default)` and stays last. The default Claude target always stays separate. One deterministic successful quota snapshot is selected without averaging. Missing, malformed, conflicting, or deliberately untrusted display identity does not discard valid quota: the row says `identity unavailable`, is partial, and causes exit code 1. Successful default Claude quota therefore remains visible, contributes one unavailable account, and points Next at `multisubs doctor`.
...
Claude routing scores managed profiles only, using fresh official session and weekly all-model usage. It includes the Fable window only when that candidate's effective CLI and settings state says Fable is applicable or possible. The three-state policy fails closed per candidate: uncertainty requires Fable capacity but does not fail routing for other managed candidates. The default is never probed or selected by automatic `multisubs claude exec`.
...
Each managed candidate uses its profile-local user settings root. Selected project and local settings, explicit `--settings`, and local macOS managed files are merged for that candidate without reading credentials or executing policy helpers. Server-managed, account, organization, or operating-system policy values that cannot be proved locally stay uncertain at field level. A conclusive higher-precedence CLI value can make an unknown lower value irrelevant. Explicit default execution remains provider-owned, and automatic routing does not inspect its settings.

Usage probe failure excludes only the affected managed candidate. Organization deduplication and reservation locking apply to every managed candidate. The default account enters neither mechanism. The tool does not infer usage from credential contents.
...
Both default accounts remain outside product ownership. Automatic routing never manages their credentials or configuration. An explicit provider CLI command may change provider-owned state only when the user requests that command. The official default Codex app server may write only its normal non-credential operational state while reporting usage.
...
No real person's email address or name belongs in this repository. Test and example identities use reserved domains, currently `example.com`, `example.net`, `example.org`, and any `.test`, `.example`, `.invalid`, or `.localhost` name, with synthetic local parts. The sensitive-text policy rejects any other email domain in changed files, commit messages, patches, and pull request text, and redacts the address in its own output. Real names cannot be detected automatically and remain a review responsibility. No-reply identities, such as a `users.noreply.github.com` address or a `noreply@` local part, are accepted because they are structurally non-deliverable and appear legitimately in `Co-authored-by` trailers. Each address is extracted and compared fully anchored against the allowed sets, so a real address at a domain whose label merely begins with a reserved word - `example-evil.com`, `testing.net`, `invalid-corp.com` - is rejected rather than mistaken for a fixture. Quoted local parts are recognised.

Commit identity is enforced separately from content. Rewriting file contents never touches the author and committer headers, so a misconfigured `user.email` publishes a real address that the content rules cannot see. Before a push leaves the machine, every author and committer email in the outbound range must match an entry in `scripts/security/commit-identity-allowlist.txt`; missing, malformed, and unlisted addresses are rejected, and the offending address is redacted in the failure output. The reserved test domains are deliberately not accepted as authorship. CI re-checks every identity in the repository's history so a commit made without the hook installed cannot pass unnoticed.

The hook only protects a machine where it is installed. Run `pre-commit install` in every clone; without it none of these checks run locally.
