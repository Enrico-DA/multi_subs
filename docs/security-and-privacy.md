...
Claude usage treats an official not-logged-in auth status as `not logged in` even when the `/usage` probe returns unreadable text. That keeps status and doctor aligned and points Next at `claude auth login` instead of `multisubs doctor`.
