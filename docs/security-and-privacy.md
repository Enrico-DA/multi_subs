...
- active `MULTISUBS_*` routing and metadata controls;
- known legacy `MULTICODEX_*` controls.

The child then receives exactly the provider home required for its selected context. Default Codex execution receives no managed auth override, and default Claude execution receives no `CLAUDE_CONFIG_DIR`.
...
- Codex dry run;
- exact provider help passthrough.
