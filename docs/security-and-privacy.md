...
- `multicodex generate` uses the selected home's existing managed ChatGPT authentication through Codex App Server. It requests no token refresh, rejects API-key authentication, discards App Server standard error, and never reads or prints credential values.
- Generation workspaces and model catalogs are private temporary files. Generation uses an empty working directory, empty client instructions, disabled context and tools, an ephemeral thread, read-only sandboxing, and fail-closed handling for action requests or unexpected items.
