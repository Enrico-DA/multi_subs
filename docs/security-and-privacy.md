...
- `multicodex generate` uses the selected home's existing managed ChatGPT authentication through Codex App Server and pins Codex's built-in OpenAI provider. It ignores unrelated custom model providers, fails closed if config replaces the built-in provider or its endpoint, requests no token refresh, rejects API-key authentication, discards App Server standard error, and never reads or prints credential values.
