...
`multicodex` is a local-first CLI that helps users manage multiple Codex and Claude subscription accounts on one machine without repeated sign-out and sign-in churn.
...
- Do not manage either system default auth account.
- Show live subscription usage across compatible local accounts from the same product.
- Route headless Codex and Claude work through provider-specific, quota-aware selectors.
