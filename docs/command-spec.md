...
- Creates one single-pane tmux session for each window. The dedicated tmux server disables status, mouse, renaming, prefix keys, prefix bindings, and child clipboard escapes; enables focus events, extended modifier keys, and copy mode; and keeps 50,000 history lines. The terminal application owns system clipboard selection. The outer editor shows one attached terminal at a time while every other session stays live and is polled.
...
- Uses conventional controls instead of a mnemonic command-key mode. `Ctrl+G` focuses the sidebar; arrows, Home, End, Page Up, and Page Down move selection; `Enter` opens a window; `Tab` opens a visible Actions menu; `F1` shows help; and `Esc` returns to terminal input. Actions and destructive confirmations use arrow or Tab selection followed by `Enter`. The Actions menu can send a literal `Ctrl+G` to the attached terminal.
