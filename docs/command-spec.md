...
- Creates one single-pane tmux session for each window. The dedicated tmux server disables status, renaming, prefix keys, prefix bindings, and child clipboard escapes; enables mouse input, focus events, extended modifier keys, and copy mode; and keeps 50,000 history lines. The outer editor forwards terminal-region clicks and wheel events to tmux. Terminal text selection uses the terminal application's mouse-reporting override. The outer editor shows one attached terminal at a time while every other session stays live and is polled.
- Window creation presents a direct Shell or Codex choice instead of accepting a free-text launch value. A Shell window opens the host's normal shell. A Codex window runs `multicodex cli` on that host, so that host's profiles, auth, and routing rules apply.
- Creates every Git workspace as a private editor-owned worktree with a new `multicodex/<slug>-<id>` branch. It fetches the detected remote default branch before creation when available and does not update or switch the source checkout's local base branch. It rejects nested, bare, unavailable, or uncertain Git roots.
- Uses a non-Git project in place and permits only one workspace for it. It never removes the project directory.
- Shows only projects with at least one workspace. A workspace stays visible with no window. It orders projects by observed terminal activity and assigns `Alt+1` through `Alt+9` dynamically from current window display order. Command-key slots are best effort because terminal applications control those keys.
- Keeps a labeled weekly-usage band visible above the workspace and terminal regions. It shows every configured local Codex account without a hidden remainder, including loading, unavailable, and stale states. A failed refresh preserves the last successful values as stale. If the complete band and usable controls cannot fit, it reports the required terminal height instead of hiding accounts.
- Draws labeled boundaries around weekly usage, workspaces, and the terminal. The header identifies the active input region. Empty states, forms, dialogs, and the footer give direct mouse and keyboard instructions for the current task.
- Uses conventional mouse and keyboard controls instead of a mnemonic command-key mode. A click opens a sidebar window, activates a visible header button or menu item, selects a form field, or chooses a dialog button. The wheel moves sidebar and menu selection and scrolls tmux terminal history. Clicking the terminal returns input focus to it. `Ctrl+G` focuses the sidebar; arrows, Home, End, Page Up, and Page Down move selection; `Enter` opens a window; `Tab` opens Actions; `F1` shows help; and `Esc` returns to terminal input. Actions and destructive confirmations also use arrow or Tab selection followed by `Enter`. The Actions menu can send a literal `Ctrl+G` to the attached terminal.
...
- Runs safe cleanup at startup and hourly. The seven-day cleanup removes only exact owned dead windows, clean worktrees, branches with no commits beyond their recorded base, and expired attachments. It never removes live, dirty, unique, uncertain, unowned, or altered resources.
