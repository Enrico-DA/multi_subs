...
- Accepts only system SSH alias names. SSH uses normal host-key policy, batch mode, bounded connection and keepalive timeouts, no listener, one private control socket, and one long-lived host protocol connection per reachable host.
...
- Creates one single-pane tmux session for each window. The dedicated tmux server disables status, mouse, renaming, prefix keys, and prefix bindings; enables focus events, extended modifier keys, and clipboard integration; and keeps 50,000 history lines. The outer editor shows one attached terminal at a time while every other session stays live and is polled.
...
- Shows only projects with at least one workspace. A workspace stays visible with no window. It orders projects by observed terminal activity and assigns `Alt+1` through `Alt+9` dynamically from current window display order. Command-key slots are best effort because terminal applications control those keys. `Ctrl+G`, then `g`, sends a literal `Ctrl+G` to the attached terminal.
- Passes ordinary terminal paste to the selected terminal. Clipboard-image and file attachment actions read at most 16 MiB from a regular client file, copy it to a private owned path on the selected host, and paste only that path into the terminal draft.
