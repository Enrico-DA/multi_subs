...
3. `Claude`: Claude binary, provider registry, managed paths, authentication status, managed duplicate-organization checks, and an identity-verification warning for a logged-in default account.
...
A missing optional window is `not reported`. A failed account is `unavailable` with a fixed safe reason. If identity is missing, malformed, or conflicted while quota succeeds, the quota row is retained as a separate row, its identity is `identity unavailable`, and it contributes one unavailable count because no safe logical collapse is proven. Successful default Claude quota always has this outcome because cached status metadata cannot be bound to the live credential context used by `/usage`. Its separate row makes the report partial, exits with code 1, and uses `multisubs doctor` as its recovery command. A safe Codex session can remain visible with a `partial` reason when required weekly data is unavailable. Source cleanup failure uses a fixed safe reason and fails that account. Every success is still printed. When the result is not complete, a `Next` section lists each distinct exact recovery command once using the same closed allow-list as `multisubs status`. Exit code 0 means every logical account probe and identity check succeeded, exit code 1 means at least one account, identity, or provider failed, and exit code 2 means invocation misuse. No arguments or flags are accepted. `--json` is not available in this release.
...
Runs official `claude auth login --claudeai` with the managed profile's derived `CLAUDE_CONFIG_DIR`. It verifies subscription auth and rejects an organization already used by another managed profile. It does not compare the default account because that account is never an automatic routing candidate.

Verification requires all of: logged-in status, `claude.ai` auth method, first-party API provider, a `max` subscription, and a non-empty organization ID. Any other subscription plan, including Pro, fails verification, so managed Claude profiles currently require Claude Max. The same rule decides which managed profiles `multisubs claude exec` may route to, and it makes a managed target's Claude doctor check warn. Claude usage reporting is deliberately exempt: it shows quota for any logged-in managed profile and quota without trusted identity for the logged-in default account.
...
Runs official Claude print mode after fresh target-scoped auth and usage checks for managed profiles.

- Only usable managed profiles enter the automatic candidate list. The default account is never probed or selected by this command.
...
- Effective settings are resolved for each managed candidate. Its user source is `<profile.ConfigDir>/settings.json`.
...
- Duplicate managed organizations are removed before execution.
...
- The unmanaged default remains explicitly runnable with `multisubs claude cli default`; that command supplies no `CLAUDE_CONFIG_DIR`.

### `multisubs claude status`

Uses official `claude auth status --json` for the default account and each managed profile. Managed profiles may show the validated provider identity and compact auth description. For the default account, only logged-in or logged-out credential presence is used. A logged-in default prints `identity unavailable` and hides the cached email, organization, and auth description because they cannot be bound to the live credential store. It adds no Next step while logged in. Probe failures print a fixed safe reason instead of raw provider text. When a target is not logged in or cannot be checked, a `Next` section prints the exact command using the same closed allow-list as `multisubs status`. It accepts no extra arguments.

### `multisubs claude usage`

Prints the Claude-only view of the shared usage report. For each managed profile, one bounded context covers official `claude auth status --json`, the free non-persistent `/usage` probe, and a second official auth status. If the first official auth result reports that the account is not logged in, the row is `not logged in` and `/usage` does not run. If the second official auth result reports that the account is not logged in, the row is also `not logged in`, even when the usage probe text cannot be parsed.

Managed usage identity requires logged-in status, a non-empty organization ID, and a strictly normalized email; it does not apply Max, provider, or auth-method routing restrictions. Managed grouping is allowed only when organization ID and normalized email are unchanged across both status results. Identity change or failure keeps valid quota as an ungrouped `identity unavailable` partial row. Managed targets with the same stable organization collapse into one logical row; different non-empty managed organization IDs never merge by email.

The default account runs only the bounded `/usage` probe and never queries cached identity during usage collection. A provider result that says login is required becomes `not logged in`; valid quota remains visible in a separate `default · identity unavailable` row with `partial · identity unavailable`. It contributes one unavailable account, makes the command exit with code 1, and uses `multisubs doctor` as its recovery command. Identical commands in the `Next` section are printed once. The default never groups with a managed profile.
...
Runs only the focused Claude binary, registry, path, authentication, and managed duplicate-organization checks. A logged-in default account produces a warning that its identity cannot be verified. Its cached email and organization are hidden and do not enter duplicate checks. That warning alone does not fail doctor; managed duplicate organizations still fail it. The command is read-only and accepts no extra arguments.
