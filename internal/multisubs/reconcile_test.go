...
func TestReconcileAppliesResourcesForAllProfiles(t *testing.T) {
	...
	out, err := captureStdout(t, func() error { return app.Run([]string{"codex", "reconcile"}) })
	if err != nil {
	...
	second, err := captureStdout(t, func() error { return app.Run([]string{"codex", "reconcile"}) })
	if err != nil {
...
func TestReconcileRejectsUnsafeSystemSkillLinksWithoutMutation(t *testing.T) {
	for _, targetKind := range []string{"external", "broken"} {
		t.Run(targetKind, func(t *testing.T) {
			...
			_, err := captureStdout(t, func() error { return app.Run([]string{"codex", "reconcile"}) })
...
func TestReconcileDoesNotRunCodexOrTouchAuth(t *testing.T) {
	...
	if _, err := captureStdout(t, func() error { return app.Run([]string{"codex", "reconcile"}) }); err != nil {
		t.Fatalf("reconcile: %v", err)
	}
...
func TestReconcileRejectsArguments(t *testing.T) {
	app := newTestAppForCLI(t)
	err := app.Run([]string{"codex", "reconcile", "work"})
	var exitErr *ExitError
