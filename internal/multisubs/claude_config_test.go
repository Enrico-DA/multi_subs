...
func TestClaudeAddUsesSeparateV1SidecarAndPrivateDerivedPath(t *testing.T) {
	...
	wantConfigDir := filepath.Join(app.store.paths.MultisubsHome, "providers", "claude", "profiles", "work", "config")
	if profile.ConfigDir != wantConfigDir {
		t.Fatalf("config dir: got %q want %q", profile.ConfigDir, wantConfigDir)
	}
	for _, path := range []string{
		app.store.paths.MultisubsHome,
		filepath.Join(app.store.paths.MultisubsHome, "providers"),
...
func TestClaudeAddRejectsProtectedDefaultName(t *testing.T) {
	...
	if !strings.Contains(exitErr.Message, "cannot be managed") {
