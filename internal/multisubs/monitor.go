...
import (
	...
	"github.com/Enrico-DA/multi_subs/internal/monitor/tui"
...
func (a *App) runMonitorTUI(args []string) error {
	...
	if err := fs.Parse(args); err != nil {
		...
		return &ExitError{Code: 2, Message: "usage: multisubs codex monitor [--interval 60s] [--timeout 60s] [--no-color] [--no-alt-screen] [--include-default] [--include-active] [--discover]"}
	}
	if fs.NArg() != 0 {
		return &ExitError{Code: 2, Message: "usage: multisubs codex monitor [--interval 60s] [--timeout 60s] [--no-color] [--no-alt-screen] [--include-default] [--include-active] [--discover]"}
	}
...
func printMonitorUsage() {
	...
	fmt.Println()
	fmt.Println("Show live Codex subscription usage across multisubs profiles and compatible local accounts.")
	fmt.Println("The monitor is read-only and does not mutate Codex account data.")
	fmt.Println()
	fmt.Println("Usage:")
	fmt.Println("  multisubs codex monitor                       Run terminal user interface (default)")
	fmt.Println("  multisubs codex monitor tui [flags]           Run terminal user interface explicitly")
