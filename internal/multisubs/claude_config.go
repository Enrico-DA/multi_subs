...
func withClaudePaths(paths Paths) Paths {
	if paths.ClaudeProviderDir == "" {
		paths.ClaudeProviderDir = filepath.Join(paths.MultisubsHome, "providers", "claude")
	}
...
func (s *claudeStore) EnsureBaseDirs() error {
	...
	paths := []string{s.paths.MultisubsHome, providersDir, s.paths.ClaudeProviderDir, s.paths.ClaudeProfilesDir}
...
func (s *claudeStore) validateProviderTree() error {
	providersDir := filepath.Dir(s.paths.ClaudeProviderDir)
	for _, path := range []string{s.paths.MultisubsHome, providersDir, s.paths.ClaudeProviderDir, s.paths.ClaudeProfilesDir} {
...
func (s *claudeStore) validateProfileStoragePath(profile claudeProfile) error {
	...
	for _, path := range []string{s.paths.MultisubsHome, filepath.Dir(s.paths.ClaudeProviderDir), s.paths.ClaudeProviderDir, s.paths.ClaudeProfilesDir, filepath.Dir(expected), expected} {
		if err := ensureClaudePrivateDir(path, false); err != nil {
			return err
		}
	}
	if err := ensurePathPrefixesBelowRootNotSymlinks(s.paths.MultisubsHome, expected); err != nil {
...
func validateClaudeProfileName(name string) error {
	...
	if name == claudeDefaultTarget {
		return fmt.Errorf("profile name %q identifies the built-in Claude default account and cannot be managed", name)
	}
	return nil
}
