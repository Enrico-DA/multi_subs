...
func renderDryRunOverview(store *Store, cfg *Config, resolved *resolvedProfileResources) string {
	...
	b.WriteString("1. init creates local multisubs directories and config only.\n")
	...
	b.WriteString("5. exec routes codex exec across the default account and managed profiles using the same weekly policy, and confirms default-account login before launching it.\n")
...
func renderDryRunLogin(cfg *Config, name, resourcePlan string) (string, error) {
	...
	var b strings.Builder
	b.WriteString("multisubs codex dry-run login\n")
	...
	b.WriteString(" codex login -c ")
	b.WriteString(shellQuoteValue(managedCodexAuthConfig))
	b.WriteString("\n")
