...
func (a *App) cmdHeartbeat(args []string) error {
	if len(args) != 0 {
		return &ExitError{Code: 2, Message: "usage: multisubs codex heartbeat"}
	}
	...
	if !acquired {
		fmt.Println("multisubs codex heartbeat")
		...
		if err := ensureProfileCodexExecutionReady(a.store.paths, profile); err != nil {
			...
			if !hasAuth && errors.Is(authStatErr, os.ErrNotExist) {
				rows = append(rows, heartbeatRow{
					Profile: name,
					Status:  "skipped",
					Detail:  "auth.json not found. run multisubs codex login <name>",
				})
				continue
			}
			failed++
...
func loadHeartbeatSettings(paths Paths) (heartbeatSettings, error) {
	settings := heartbeatSettings{
		...
		LockPath: filepath.Join(paths.MultisubsHome, "heartbeat.lock"),
	}

	timeoutSeconds, err := parsePositiveEnvInt("MULTISUBS_HEARTBEAT_TIMEOUT_SECONDS", int(settings.Timeout/time.Second))
	if err != nil {
		return heartbeatSettings{}, err
	}
	settings.Timeout = time.Duration(timeoutSeconds) * time.Second

	retries, err := parseNonNegativeEnvInt("MULTISUBS_HEARTBEAT_RETRIES", settings.Retries)
	if err != nil {
		return heartbeatSettings{}, err
	}
	settings.Retries = retries

	backoffSeconds, err := parseNonNegativeEnvInt("MULTISUBS_HEARTBEAT_BACKOFF_SECONDS", int(settings.Backoff/time.Second))
	if err != nil {
		return heartbeatSettings{}, err
	}
	settings.Backoff = time.Duration(backoffSeconds) * time.Second

	if value := strings.TrimSpace(os.Getenv("MULTISUBS_HEARTBEAT_LOCK_PATH")); value != "" {
		lockPath, err := resolvePathInsideRoot(paths.MultisubsHome, value, "heartbeat lock path")
		if err != nil {
			return heartbeatSettings{}, err
		}
