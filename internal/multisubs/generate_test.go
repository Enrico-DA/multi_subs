...
func TestReadBoundedPrompt(t *testing.T) {
	...
	if _, err := readBoundedPrompt(generateErrorReader{}); err == nil {
...
func TestCmdGenerateRoutesAndWritesOnlyGeneratedText(t *testing.T) {
	...
	output, err := captureStdout(t, func() error {
		return app.Run([]string{"codex", "generate", "--model", "gpt-test", "hello"})
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCmdGenerateUsesExplicitProfile(t *testing.T) {
	...
	if err := app.Run([]string{"codex", "generate", "--account", "alpha", "hello"}); err != nil {
		t.Fatal(err)
	}
	if err := app.Run([]string{"codex", "generate", "--account", "missing", "hello"}); err == nil {
...
func TestCmdGeneratePassesCustomOptions(t *testing.T) {
	...
	generateWithSubscription = func(_ context.Context, options codexappserver.GenerateOptions) error {
		if options.ActiveProfile != "alpha" || options.Model != "gpt-test" || options.Effort != "high" ||
			...
			string(options.OutputSchema) != `{"type":"object"}` || !options.JSONOutput ||
			!options.WebSearch || options.Prompt != "hello" {
	...
	err := app.Run([]string{
		"codex", "generate", "--account", "alpha", "--model", "gpt-test", "--effort", "high",
...
type generateErrorReader struct{}

func (generateErrorReader) Read([]byte) (int, error) {
