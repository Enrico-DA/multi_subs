...
func TestRunCodexDoctorWarnsWhenGenerateVersionDoesNotMatch(t *testing.T) {
	...
	for _, check := range report.Checks {
		...
		if !strings.Contains(check.Details, "codex-cli 0.144.4") ||
			!strings.Contains(check.Details, "generate requires "+codexappserver.PreviousSupportedCodexVersion) ||
			!strings.Contains(check.Details, codexappserver.SupportedCodexVersion) {
...
func TestRunCodexDoctorAcceptsSupportedGenerateVersion(t *testing.T) {
	for _, version := range []string{
		codexappserver.SupportedCodexVersion,
		codexappserver.PreviousSupportedCodexVersion,
	} {
		t.Run(version, func(t *testing.T) {
			...
			script := "#!/bin/sh\nprintf '%s\\n'\n"
			script = fmt.Sprintf(script, version)
			if err := os.WriteFile(filepath.Join(fakeBin, "codex"), []byte(script), 0o755); err != nil {
				t.Fatalf("write fake codex: %v", err)
			}
			t.Setenv("PATH", fakeBin+string(os.PathListSeparator)+os.Getenv("PATH"))
			...
			for _, check := range report.Checks {
				...
				if !strings.Contains(check.Details, version) {
...
func TestIsSensitiveTrackedPath(t *testing.T) {
	...
	cases := []struct {
		...
		{path: ".multisubs/providers/claude/config.json", sensitive: true},
		{path: "workspace/.multisubs/providers/claude/config.json", sensitive: true},
		{path: ".multicodex/config.json", sensitive: true},
		{path: ".multicodex/profiles/work/codex-home/config.toml", sensitive: true},
		{path: ".multicodex/providers/claude/config.json", sensitive: true},
		{path: "workspace/.multicodex/providers/claude/config.json", sensitive: true},
		{path: "github.com/olliecrow/.multicodex/config.json", sensitive: true},
