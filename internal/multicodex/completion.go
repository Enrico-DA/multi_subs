...
_multicodex_complete() {
  ...
  local commands="init add login login-all cli exec generate status reconcile heartbeat monitor doctor dry-run completion version help"
  ...
  case "$cmd" in
    ...
    help)
      if (( COMP_CWORD == 2 )); then
        COMPREPLY=( $(compgen -W "init add login login-all cli exec generate status reconcile heartbeat monitor doctor dry-run completion version help monitor\ doctor monitor\ completion monitor\ tui" -- "$cur") )
        return 0
      fi
      ;;
  esac
...
_multicodex_complete() {
  ...
  case "$cmd" in
    ...
    help)
      if (( CURRENT == 3 )); then
        compadd -- init add login login-all cli exec generate status reconcile heartbeat monitor doctor dry-run completion version help "monitor doctor" "monitor completion" "monitor tui"
        return
      fi
      ;;
  esac
...
complete -c multicodex -f -n '__fish_use_subcommand' -a 'init add login login-all cli exec generate status reconcile heartbeat monitor doctor dry-run completion version help'
