...
func TestExplicitSkillSourcesRejectProtectedStateBoundaries(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "another profile home", source: otherProfileHome, want: "overlaps multicodex-owned state"},
		{name: "ancestor containing protected state", source: filepath.Dir(store.paths.MulticodexHome), want: "overlaps multicodex-owned state"},
	...
	if err == nil || !strings.Contains(err.Error(), "overlaps multicodex-owned state") {
