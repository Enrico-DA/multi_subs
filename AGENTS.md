...
- Never change, copy, restore, back up, symlink, or otherwise manage either shared default auth account. The default Codex account participates normally in routing while remaining outside product ownership. The default Claude account remains usage-visible and explicitly runnable with `multisubs claude cli default`, but it never participates in automatic routing or trusted identity and organization reconciliation. An official app-server usage probe may write non-credential logs, caches, and database files in the default Codex home, but it must not change default credentials.
...
- Never commit a real person's email address or name. Test and example identities use reserved domains such as `example.com` or `example.test`, with synthetic local parts. The sensitive-text policy enforces the domain rule on changed files, commit messages, patches, and pull request text; the name rule relies on review. No-reply identities are accepted in trailers because they are not a person's mailbox.
- Commit author and committer identity is separately enforced. Every identity on an outbound push must match `scripts/security/commit-identity-allowlist.txt`; use a GitHub no-reply address rather than a real mailbox. Content rules do not cover identity headers, so this is a distinct check.
...
- Keep automatic Claude routing limited to managed profiles and based on fresh official session, weekly all-model, and Fable usage without reading credential contents.
